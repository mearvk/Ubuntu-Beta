...
# Install the Orange Edition dconf source policy into an ISO root.
...
if [ ! -d "$ROOT_DIR/profile" ] || [ ! -d "$ROOT_DIR/db/local.d" ]; then
  echo "ERROR: incomplete Orange Edition configuration tree." >&2
  exit 1
fi

install -D -m 0644 "$ROOT_DIR/profile/user" \
  "$TARGET_ROOT/etc/dconf/profile/user"
install -D -m 0644 "$ROOT_DIR/db/local.d/00-orange-edition" \
  "$TARGET_ROOT/etc/dconf/db/local.d/00-orange-edition"
