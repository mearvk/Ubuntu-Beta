...
# Install the human-readable Orange Edition dconf configuration into an ISO
...
command -v dconf >/dev/null 2>&1 || {
  echo "ERROR: dconf is required to compile the Orange Edition configuration." >&2
  exit 1
}
...
install -m 0644 "$ROOT_DIR/db/local.d/00-orange-edition" "$TARGET_ROOT/etc/dconf/db/local.d/00-orange-edition"
...
echo "Ubuntu Orange Edition GNOME configuration installed in: $TARGET_ROOT"
